...
1. **CORRECTED (Jul 4): `validate_stat` DOES exist on the txoracle program (devnet included).** Earlier drafts assumed it did not; reading the official docs + `github.com/txodds/tx-on-chain` confirmed the brief was accurate. TxLINE posts **Merkle roots on-chain** (scores every 5 min, fixtures daily) and `validate_stat` verifies stat predicates against them (~1.4M CU via `.view()`). Our program (`kick-settlement`, deployed spec in `docs/technical/SMART-CONTRACT.md §5`) uses a rotatable `VerifyMode`: hash-anchor (active), in-program Merkle walk (shipped, awaiting hash-algo confirmation), CPI into `validate_stat` (reserved pending CU headroom testing). Remaining Q1 residue: only the **hash algorithm** needs confirming in Telegram.
